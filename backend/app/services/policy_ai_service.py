...
All LLM traffic routes through :mod:`app.services.ai_gateway` so we
inherit provider routing, retries, timeouts, and response caching.
"""
...
from app.services.ai_gateway import ai_complete, has_provider
...
def _local_recommendation(policy_type: str, cve: CVE, profile: OrgProfile) -> tuple[str, str]:
    ...
    if cve.is_kev:
        why_bits.append(
            "it appears on the CISA Known Exploited Vulnerabilities catalog "
    ...
    if "remote" in desc or "network" in desc or "rce" in desc:
        why_bits.append(
            "language in the CVE narrative points to network-exposed or remote attack paths"
        )
    if not why_bits:
        why_bits.append(
            "the CVE metadata and severity signal organizational risk that should be governed under policy"
        )
...
_SYSTEM = (
    "You are a senior information security architect. Produce a concise policy "
    "recommendation for the customer's environment. You MUST include two labeled "
    "sections exactly as follows:\n"
    "RECOMMENDATION: (actionable guidance)\n"
    "WHY_THIS_MAPS: (2–4 sentences explaining why this CVE belongs under the "
    "selected policy type, citing concrete phrases, CWEs, KEV status, or "
    "exposure — not generic filler).\n"
...
async def generate_policy_recommendation(
    ...
    if not has_provider():
        rec, why = _local_recommendation(policy_type, cve, profile)
        return rec, why, "local-rules"
    ...
    result = await ai_complete(
        ...
        timeout=25.0,
        cache_ttl=300,
        label=f"policy_ai:{policy_type}",
    )

    if not result.ok:
        logger.info(
            "Policy LLM generation unavailable (%s); using local fallback",
            result.error,
        )
        rec_l, why_l = _local_recommendation(policy_type, cve, profile)
        return rec_l, why_l, "local-fallback"

    rec, why = _split_llm_text(result.text)
    if not why:
        rec2, why2 = _local_recommendation(policy_type, cve, profile)
        return (rec or rec2), why2, f"{result.model}+filled-why"
    return rec, why, result.model
